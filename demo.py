    ...
    try:
        ...
        if st.button("Start Inference"):
            with st.spinner("Processing your data..."):
                ...
                df_with_predictions = predict_csv_onnx(df_uploaded, model_path, tokenizer_path, progress_bar)
                ...
                for t,address in enumerate(df_with_predictions["address"]):
                    ...
                    rows.append(table['city_name','district_name','quarter_name', 'street_name'].to_dicts()[0])

                ner_output_df = pd.DataFrame(rows)
                df_with_predictions = df_with_predictions.merge(ner_output_df, left_index=True, right_index=True,)
                end_time = time.perf_counter()
                elapsed_time = end_time - start_time
