...
train_dataloader = DataLoader(train_dataset, batch_size=128, shuffle=True,  pin_memory=True, num_workers=2)
val_dataloader = DataLoader(val_dataset, batch_size=128, shuffle=False, pin_memory=True, num_workers=2)
...
def train(
    ...
    if resume and os.path.exists(checkpoint_path):
        ...
        if verbose:
            print(f"[resume] {start_epoch}.(best_val_loss={best_loss:.6f}, "
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        for i,batch in enumerate(tqdm(train_dataloader, desc=f"Epoch {epoch} [TRAIN]")):
            ...
            if (i + 1) % accumulation_steps == 0:
                ...
                torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm) if max_norm is not None else None
        ...
        if no_improve_count >= patience:
            if verbose:
                print(f"Early stopping @ epoch {epoch+1} (best_val_loss={best_loss:.6f})")
